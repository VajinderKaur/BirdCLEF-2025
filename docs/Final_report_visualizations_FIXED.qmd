...
```{python eval=TRUE, context='global', echo = FALSE}
import os
import gc
import warnings
import logging
...
(See Block 4 in appendix for code)
...
(See Block 5 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
## Configuration Setup
...
(See block 6 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
...
(See block 7 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
taxonomy_df['class_name'].value_counts().plot(kind='bar', color='teal')
plt.title("Species Counts by Class")
plt.ylabel("Count")
...
(See block 8 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
## Model Definition
...
(See block 9 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
...
```{python, echo = FALSE, context = 'global'}
...
```{python, context='global', echo = FALSE}
import numpy as np
import librosa
import torch
import torch.nn.functional as F
...
# Model Finding
(See block 10 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
...
(See block 11 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
...
(See block 12 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
...
(See block 13 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
def main():
    import types
    cfg = types.SimpleNamespace(
...
(See block 14 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
import os
import pandas as pd
import matplotlib.pyplot as plt
...
```{python eval = FALSE, context = 'global', echo = FALSE}
if __name__ == "__main__":
    main()
```
...
(See block 15 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
...
```{python eval = TRUE, echo = FALSE, context = 'global'}
import pandas as pd
import matplotlib.pyplot as plt
...
(See block 16 in appendix for code)
```{python eval=TRUE, context='global', echo = FALSE}
...
(See block 17 in appendix for code)
```{python, eval = TRUE, context = 'global', echo = FALSE}
import seaborn as sns
import matplotlib.pyplot as plt
...
```
(See block 18 in appendix for code)
...
This appendix contains the full code used for data loading, prediction analysis, and visualizations. These code blocks are included for reference only and are **not executed** during report rendering.
...
tryCatch({
  ...
  tryCatch(py_install("torch", pip = TRUE), error = function(e) message("Error installing torch: ", e$message))
  tryCatch(py_install("librosa", pip = TRUE), error = function(e) message("Error installing librosa: ", e$message))
  tryCatch(py_install("opencv-python-headless", pip = TRUE), error = function(e) message("Error installing opencv: ", e$message))
...
### Block 2 (Python Diagnostic Setup)
```python
import sys
import os
...
class CFG:
    ...
    test_soundscapes = "/path/to/test_soundscapes"
    submission_csv = "/path/to/sample_submission.csv"
...
### Block 7 (Taxonomy Visualization)
```python
taxonomy_df['class_name'].value_counts().plot(kind='bar', color='teal')
plt.title("Species Counts by Class")
plt.ylabel("Count")
